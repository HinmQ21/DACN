"""Script to run benchmark evaluation using a single LLM (without multi-agent workflow)."""

import argparse
from benchmarks.single_llm_eval import SingleLLMMedQAEvaluator, SingleLLMPubMedQAEvaluator
...
def print_metrics(metrics: dict, dataset_name: str):
    ...
    print("\n" + "="*60)
    print(f"{dataset_name} EVALUATION RESULTS (SINGLE LLM)")
    print("="*60)
...
def main():
    parser = argparse.ArgumentParser(description="Run Medical QA Benchmark Evaluation with Single LLM")
    parser.add_argument(
        '--dataset',
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Maximum number of samples to evaluate (default: from config or all)'
    )
    parser.add_argument(
        '--model',
        type=str,
    ...
    print(f"\n{'='*60}")
    print("SINGLE LLM BENCHMARK EVALUATION")
    print(f"{'='*60}")
    ...
    if args.dataset in ['medqa', 'both']:
        ...
        evaluator = SingleLLMMedQAEvaluator(
    ...
    if args.dataset in ['pubmedqa', 'both']:
        ...
        evaluator = SingleLLMPubMedQAEvaluator(
            model_name=model_name,
            temperature=temperature
        )
        try:
    ...
    if save_results and results_summary['results']:
        summary_path = f"benchmark_summary_single_llm_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
